...
public class KinectManager : MonoBehaviour {
    ...
    void Update() {
        if (kinectInitialized) {
            ...
            while (alUserIds.Count > bodyParts.Count) {
                bodyParts.Add(new BodyParts());

                BodyParts instance = new BodyParts();
                instance.legs = new Transform[2];

                instance.legs[0] = Instantiate(legPref).transform;
                instance.legs[1] = Instantiate(legPref).transform;
            ...
            for (int i = 0; i < alUserIds.Count; i++) {
                ...
                if (index >= 0 && index < sensorData.bodyCount) {
                    DrawSkeleton(usersLblTex, ref bodyFrame.bodyData[index], i);
    ...
    protected virtual void RemoveUser(Int64 userId) {
        ...
        bodyParts[uidIndex].legs[0].gameObject.SetActive(false);
        bodyParts[uidIndex].legs[1].gameObject.SetActive(false);
    ...
    private void DrawSkeleton(Texture2D aTexture, ref KinectInterop.BodyData bodyData, int userId) {
               ...
               bodyData.joint[parent].trackingState != KinectInterop.TrackingState.NotTracked) {
                ...
                if (posParent != Vector2.zero && posJoint != Vector2.zero) {
                    ...
                    if (i == 15) { //The two joints I need. Use these two to create feet.
                        ...
                        temp.y = 0;
                        bodyParts[userId].legs[0].position = temp;
                    ...
                    if (i == 19) {
                        ...
                        temp = bodyData.joint[i].kinectPos;
                        temp.y = 0;
                        bodyParts[userId].legs[1].position = temp;
